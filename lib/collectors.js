...
async function collectUSStocks() {
  ...
  // PER/PBR/배당률/시총 수집 (Yahoo Finance quoteSummary)
  console.log('[Collect] 미국 종목 재무지표 수집 시작...');
  for (let i = 0; i < results.length; i += BATCH_SIZE) {
    const batch = results.slice(i, i + BATCH_SIZE);
    const fundPromises = batch.map(async (stock) => {
      try {
        const url = `https://query1.finance.yahoo.com/v10/finance/quoteSummary/${stock.symbol}?modules=defaultKeyStatistics,summaryDetail`;
        const data = await fetchJSON(url, 15000);
        ...
        // 재무지표 실패해도 가격 데이터는 유지
      }
    });
    await Promise.allSettled(fundPromises);
    if (i + BATCH_SIZE < results.length) await sleep(1000);
  }
