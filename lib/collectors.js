...
async function collectYouTube() {
  const queries = ['주식 투자 전망 2026', '증시 분석 오늘'];
  const allItems = [];
  for (const query of queries) {
    try {
      // YouTube 검색 결과 스크래핑 (sp=EgIIAQ 는 최신순 필터)
      const url = `https://www.youtube.com/results?search_query=${encodeURIComponent(query)}&sp=EgIIAQ%3D%3D`;
      ...
      console.warn(`[Collect] YouTube 검색 실패 (${query}):`, e.message);
    }
  }
  // 중복 제거
  const seen = new Set();
  const unique = allItems.filter(i => { if (seen.has(i.videoId)) return false; seen.add(i.videoId); return true; });
  console.log(`[Collect] YouTube 영상 ${unique.length}건 수집`);
...
// ===== 미국 종목 벌크 수집 =====
async function collectUSStocks() {
  const usStocks = MAIN_STOCKS.filter(s => s.market === 'NASDAQ');
  ...
  for (let i = 0; i < usStocks.length; i += BATCH_SIZE) {
    ...
    const promises = batch.map(async (stock) => {
      try {
        const url = `https://query1.finance.yahoo.com/v8/finance/chart/${stock.symbol}?interval=1d&range=3mo&includePrePost=false`;
        const data = await fetchJSON(url, 15000);
        ...
        const price = meta.regularMarketPrice;
        const dailyPrev = closes.length >= 2 ? closes[closes.length - 2] : price;
        return {
          symbol: stock.symbol,
          name: stock.name,
          sector: stock.sector,
          ...
          change: Math.round((price - dailyPrev) * 100) / 100,
          changePercent: Math.round((price - dailyPrev) / dailyPrev * 10000) / 100,
          currency: meta.currency || 'USD',
          history: closes.slice(-8).map(v => Math.round(v * 100) / 100),
          fullCloses: closes.map(v => Math.round(v * 100) / 100),
          timestamps: result.timestamp,
          live: true,
        };
      } catch (e) {
        console.warn(`[Collect] US ${stock.symbol} 실패:`, e.message);
        return null;
      }
    });
    ...
    // 배치 간 딜레이
    if (i + BATCH_SIZE < usStocks.length) await sleep(1000);
  }

  // PER/PBR/배당률/시총 수집 (Yahoo Finance cookie+crumb 인증)
  console.log('[Collect] 미국 종목 재무지표 수집 시작...');
  try {
    // 1) cookie + crumb 획득 (consent 우회 포함)
    // Step 1: consent cookie 설정 후 접근
    const initRes = await fetch('https://fc.yahoo.com', {
      headers: { 'User-Agent': BROWSER_UA },
      redirect: 'manual',
    });
    const initCookies = (initRes.headers.getSetCookie?.() || []).map(c => c.split(';')[0]).join('; ');

    // Step 2: consent 쿠키 + A3 쿠키로 crumb 획득
    const crumbRes = await fetch('https://query2.finance.yahoo.com/v1/test/getcrumb', {
      headers: { 'User-Agent': BROWSER_UA, 'Cookie': initCookies },
    });
    const crumb = await crumbRes.text();
    const cookieStr = initCookies;

    if (crumb && !crumb.includes('{')) {
      console.log(`[Collect] Yahoo crumb 획득 성공: ${crumb.slice(0, 8)}...`);
      const FUND_BATCH = 5;
      for (let i = 0; i < results.length; i += FUND_BATCH) {
        const batch = results.slice(i, i + FUND_BATCH);
        const fundPromises = batch.map(async (stock) => {
          try {
            const url = `https://query2.finance.yahoo.com/v10/finance/quoteSummary/${stock.symbol}?modules=defaultKeyStatistics,summaryDetail&crumb=${encodeURIComponent(crumb)}`;
            ...
            if (ks || sd) {
              const per = sd?.trailingPE?.raw ?? ks?.trailingPE?.raw;
              const pbr = ks?.priceToBook?.raw;
              const divYield = sd?.dividendYield?.raw;
              const mcap = sd?.marketCap?.raw;
              if (per != null && !isNaN(per)) stock.per = Math.round(per * 100) / 100;
              if (pbr != null && !isNaN(pbr)) stock.pbr = Math.round(pbr * 100) / 100;
              if (divYield != null && !isNaN(divYield)) stock.divYield = Math.round(divYield * 10000) / 100;
              if (mcap != null) {
                if (mcap >= 1e12) stock.marketCap = (mcap / 1e12).toFixed(1) + 'T';
                else if (mcap >= 1e9) stock.marketCap = (mcap / 1e9).toFixed(1) + 'B';
                else stock.marketCap = (mcap / 1e6).toFixed(0) + 'M';
              }
            }
          } catch (e) { /* 재무지표 실패해도 가격 데이터 유지 */ }
        });
        await Promise.allSettled(fundPromises);
        if (i + FUND_BATCH < results.length) await sleep(1000);
      }
    } else {
      console.warn('[Collect] Yahoo crumb 획득 실패, 재무지표 건너뜀');
    }
  } catch (e) {
    console.warn('[Collect] 미국 재무지표 전체 실패:', e.message);
  }
  const withFund = results.filter(s => s.per || s.pbr || s.marketCap).length;
  console.log(`[Collect] 미국 재무지표: ${withFund}/${results.length}개 종목 수집 완료`);

  console.log(`[Collect] 미국 종목 ${results.length}/${usStocks.length}개 수집 완료`);
  return results.length > 0 ? results : null;
}
