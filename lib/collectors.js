...
async function collectUSStocks() {
  ...
  // PER/PBR/배당률/시총 수집 (Yahoo Finance 웹 스크래핑)
  ...
  for (let i = 0; i < results.length; i += FUND_BATCH) {
    ...
    const fundPromises = batch.map(async (stock) => {
      try {
        const html = await fetchText(`https://finance.yahoo.com/quote/${stock.symbol}/key-statistics/`, 20000);
        const peMatch = html.match(/Trailing P\/E.*?<td[^>]*>([\d.]+)/s);
        const pbMatch = html.match(/Price\/Book.*?<td[^>]*>([\d.]+)/s);
        const mcMatch = html.match(/Market Cap.*?<td[^>]*>([\d.]+[TBMK]?)/s);
        const dyMatch = html.match(/Trailing Annual Dividend Yield.*?<td[^>]*>([\d.]+)%/s);
        if (peMatch) stock.per = parseFloat(peMatch[1]);
        if (pbMatch) stock.pbr = parseFloat(pbMatch[1]);
        if (dyMatch) stock.divYield = parseFloat(dyMatch[1]);
        if (mcMatch) stock.marketCap = mcMatch[1];
      } catch (e) {
        // 재무지표 실패해도 가격 데이터는 유지
      }
    });
    await Promise.allSettled(fundPromises);
    if (i + FUND_BATCH < results.length) await sleep(2000);
  }
